...
const emailFormat = function (req, res, next) {
  ...
  if (!email.match(REGEX.EMAIL)) return res.status(400).json({ message: 'Invalid email' });
...
const userToTrip = async function (req, res, next) {
  try {
    ...
    if (!exists.length) return res.status(403).json({ message: 'user not authorized or this trip' });
